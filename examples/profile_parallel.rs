...
fn main() {
    ...
    {
        use rar_stream::decompress::rar5::block_decoder::{ParallelConfig, Rar5BlockDecoder};
        ...
        eprintln!(
            "Single-threaded first 16 bytes: {:02x?}",
            &single_output[..16.min(single_output.len())]
        );

        eprintln!("Using {} threads", rayon::current_num_threads());
        eprintln!(
            "dict_size_log: {}, dict_size: {} MB",
        ...
        match result {
            Ok(output) => {
                ...
                for i in 0..output.len().min(1000) {
                    if output[i] == 0x33
                        && output.get(i + 1) == Some(&0xed)
                        && output.get(i + 2) == Some(&0x90)
                    {
                ...
                if let Some(expected) = expected {
                    if output == expected {
                        ...
                        for (i, (a, b)) in output.iter().zip(expected.iter()).enumerate() {
                            if a != b {
                                eprintln!(
                                    "MISMATCH at byte {}: got {:02x}, expected {:02x}",
                                    i, a, b
                                );
                                break;
                            }
