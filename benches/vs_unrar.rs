...
/// Benchmark 200MB Alpine ISO with RAR5 - all decode methods
#[cfg(feature = "parallel")]
fn bench_200mb_alpine_parallel(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("all_methods/200mb_alpine");
    group.throughput(Throughput::Bytes(unpacked_size as u64));
    group.sample_size(10);
    ...
    // Parallel batch (with sync barriers)
    ...
    // Pipeline (streaming, minimal sync)
    group.bench_function("parallel_pipeline", |b| {
        use rar_stream::decompress::rar5::Rar5Decoder;
        let mut decoder = Rar5Decoder::with_dict_size(dict_size_log);
        b.iter(|| {
            decoder.reset();
            let result = decoder.decompress_pipeline(black_box(compressed), unpacked_size as u64);
            black_box(result)
        });
    });

    // Official unrar reference
    group.bench_function("official_unrar", |b| {
        b.iter(|| {
            let result = extract_with_unrar_to_bytes(black_box(archive_path));
            black_box(result)
...
/// Matrix benchmark: all data types × compression settings × decode methods.
fn bench_matrix(c: &mut Criterion) {
    let matrix_dir = Path::new("__fixtures__/large/matrix");
    if !matrix_dir.exists() {
        eprintln!("Skipping matrix benchmark - directory not found");
        return;
    }

    // Discover all .rar files in matrix/
    let mut archives: Vec<_> = std::fs::read_dir(matrix_dir)
        .expect("Failed to read matrix dir")
        .filter_map(|e| e.ok())
        .filter(|e| e.path().extension().map(|x| x == "rar").unwrap_or(false))
        .map(|e| e.path())
        .collect();
    archives.sort();

    if archives.is_empty() {
        eprintln!("No archives found in matrix directory");
        return;
    }

    for archive_path in &archives {
        let stem = archive_path.file_stem().unwrap().to_str().unwrap();
        let data = std::fs::read(archive_path).expect("Failed to read archive");
        let (compressed, unpacked_size, dict_size_log) = match parse_rar5_file_with_dict(&data) {
            Some(r) => r,
            None => {
        ...
        let group_name = format!("matrix/{stem}");
        ...
        group.bench_function("single", |b| {
            ...
            b.iter(|| {
                decoder.reset();
                let result =
                    decoder.decompress(black_box(compressed), unpacked_size as u64, 1, false);
                black_box(result)
            });
        });

        // Pipeline (parallel) - only with parallel feature
        #[cfg(feature = "parallel")]
        group.bench_function("pipeline", |b| {
            ...
            b.iter(|| {
                decoder.reset();
                let result =
                    decoder.decompress_pipeline(black_box(compressed), unpacked_size as u64);
                black_box(result)
            });
        });

        // Official unrar
        group.bench_function("unrar", |b| {
            b.iter(|| {
                let result = extract_with_unrar_to_bytes(black_box(archive_path));
                black_box(result)
            });
