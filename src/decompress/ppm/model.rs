...
impl PpmModel {
    ...
    pub fn new() -> Self {
        Self {
            ...
            dummy_see2: See2Context { summ: 0, shift: PERIOD_BITS as u8, count: 0 },
            esc_char: -1,
    ...
    pub fn init(&mut self, reader: &mut BitReader) -> Result<(RangeCoder, i32), &'static str> {
        ...
        #[cfg(test)]
        eprintln!("[PPM init] max_order_byte=0x{:02x} reset={}", max_order_byte, reset);
    ...
    fn restart_model(&mut self) {
        ...
        #[cfg(test)]
        {
            for i in 0..256 {
                ...
                if sym != i as u8 {
                    eprintln!("[INIT] ERROR: stats[{}] has sym {} instead of {}", i, sym, i);
                }
            }
        }
    ...
    /// Decode a character.
    pub fn decode_char(&mut self, coder: &mut RangeCoder, reader: &mut BitReader) -> Result<i32, &'static str> {
        ...
        if self.debug_count == 0 {
            ...
            eprintln!("[ENTRY pos={}] low={} range={} code={} code-low={} prev_success={}", 
                     self.debug_count, low, range, code, code.wrapping_sub(low), self.prev_success);
        }
        ...
        if self.debug_count == 0 {
            let (code, low, range) = coder.debug_state();
            eprintln!("[ENTRY pos={}] low={} range={} code={} NumStats={} ctx={}", 
        ...
        if self.debug_count == 0 {
            ...
            eprintln!("[pos={}] min_context={} NumStats={} SummFreq={} order_fall={}", 
        ...
        if num_stats != 1 {
            ...
            if stats as usize <= text_ptr || stats as usize > heap_end {
                #[cfg(test)]
                eprintln!("[pos={}] INVALID STATS: ctx={} num_stats={} stats={} text_ptr={} heap_end={}", 
                         self.debug_count, self.min_context, num_stats, stats, text_ptr, heap_end);
                return Err("Invalid stats pointer");
            }
            #[cfg(test)]
            if self.debug_count == 0 {
                eprintln!("[pos={}] Multi-symbol context at {}, num_stats={}", self.debug_count, self.min_context, num_stats);
            }
        ...
        while self.found_state == 0 {
            ...
            if self.debug_count == 0 {
                let (code, low, range) = coder.debug_state();
                eprintln!("[ESCAPE pos={}] After normalize: low={} range={} code={}", self.debug_count, low, range, code);
            }
            ...
            loop {
                ...
                if ns as usize != self.num_masked {
                    #[cfg(test)]
                    if self.debug_count == 0 {
                        eprintln!("[ESCAPE pos={}] Found context with ns={} (masked={})", self.debug_count, ns, self.num_masked);
                    }
                    break;
                }
            }
        ...
        #[cfg(test)]
        {
            ...
            if self.debug_count == 0 || (self.debug_count >= 1120 && self.debug_count <= 1135) {
                eprintln!("[pos={}] sym='{}' ({}) bytes_consumed={} found_state={}", 
    ...
    fn decode_symbol1(&mut self, coder: &mut RangeCoder, _reader: &mut BitReader) -> Result<(), &'static str> {
        ...
        #[cfg(test)]
        if self.debug_count == 0 {
            eprintln!("[DS1 pos={}] summ_freq={} num_stats={} prev_success_before={}", 
        ...
        for i in 0..num_stats {
            ...
            #[cfg(test)]
            if self.debug_count == 0 {
                eprintln!("[DS1 pos={}] i={} sym='{}' ({}) freq={} hi_cnt={}", self.debug_count, i, sym as char, sym, freq, hi_cnt);
            }
            ...
            if hi_cnt > count {
                ...
                #[cfg(test)]
                if self.debug_count == 0 {
                    eprintln!("[DS1 pos={}] Selected i={} sym='{}' ({}) lo={} hi={}", 
                             self.debug_count, i, sym as char, sym, lo_cnt, hi_cnt);
                    let (code, low, range) = coder.debug_state();
                    eprintln!("[DS1 pos={}] BEFORE decode: low={} range={} code={}", 
                ...
                if self.debug_count == 0 {
                    let (code, low, range) = coder.debug_state();
                    eprintln!("[DS1 pos={}] AFTER decode: low={:#x} range={:#x} code={:#x}", 
                             self.debug_count, low, range, code);
                }
        ...
        #[cfg(test)]
        if self.debug_count == 0 {
            eprintln!("[DS1 pos={} ESCAPE] hi_cnt={} summ_freq={} before decode", self.debug_count, hi_cnt, summ_freq);
        }
        ...
        if self.debug_count == 0 {
            let (code, low, range) = coder.debug_state();
            eprintln!("[DS1 pos=98 ESCAPE] After decode: low={} range={} code={}", low, range, code);
        }
    ...
    fn decode_bin_symbol(&mut self, coder: &mut RangeCoder, _reader: &mut BitReader) -> Result<(), &'static str> {
        ...
        let ns_idx = if suffix_num_stats > 0 { suffix_num_stats - 1 } else { 0 };
        ...
        let freq_idx = if state.freq > 0 { (state.freq - 1) as usize } else { 0 };
        ...
        if self.debug_count == 0 {
            ...
            eprintln!("[BIN pos={} idx] idx1={} freq_idx={} bs={}", self.debug_count, idx1, freq_idx, bs);
        }
        ...
        #[cfg(test)]
        if self.debug_count == 0 {
            eprintln!("[BIN pos={}] sym='{}' ({}) freq={} bs={} count={}", 
                     self.debug_count, state.symbol as char, state.symbol, state.freq, bs, count);
            let (code, low, range) = coder.debug_state();
            eprintln!("[BIN pos={}] after get_count: low={} range={} code={}", self.debug_count, low, range, code);
        }
    ...
    fn decode_symbol2(&mut self, coder: &mut RangeCoder, _reader: &mut BitReader) -> Result<(), &'static str> {
        #[cfg(test)]
        if self.debug_count == 0 {
            let (code, low, range) = coder.debug_state();
            eprintln!("[DS2 pos={} entry] coder: low={:010} range={:010} code={:010}", 
                     self.debug_count, low, range, code);
        }
        ...
        if self.debug_count == 0 || self.debug_count == 58 {
            eprintln!("[DS2 pos={}] min_context={} num_stats={} num_masked={}", 
        ...
        if !is_root {
            ...
            let see2_idx = ns2_idx + diff_suffix + 2 * freq_check + 4 * masked_check + self.hi_bits_flag as usize;
            ...
            if self.debug_count == 58 || self.debug_count == 0 {
                let suffix = self.read_context_suffix(self.min_context as usize);
                let suffix_ns = if suffix != 0 { self.read_context_num_stats(suffix as usize) } else { 0 };
                ...
                eprintln!("[DS2 pos={}] SEE2: see2_idx={} esc_freq={}",
        ...
        if self.debug_count == 15 {
            eprintln!("[DS2 pos=15] stats pointer={}, root context={}", stats, self.min_context);
            ...
            for j in 0..num_stats {
                ...
                if self.char_mask[sym as usize] != self.esc_count {
                    ...
                    cum += freq;
                    if sym == 108 { // 'l'
                        eprintln!("[DS2 pos=15] 'l' at j={} prev_cum={} cum={} freq={}", j, prev_cum, cum, freq);
                    }
                    if sym == 112 { // 'p'
                        eprintln!("[DS2 pos=15] 'p' at j={} prev_cum={} cum={} freq={}", j, prev_cum, cum, freq);
                    }
        ...
        if self.debug_count == 13 {
            ...
            for j in 0..num_stats {
                ...
                if self.char_mask[sym as usize] != self.esc_count {
                    ...
                    cum += freq;
                    if sym == 100 { // 'd'
                        eprintln!("[DS2 pos=13] 'd' at j={} prev_cum={} cum={} freq={}", j, prev_cum, cum, freq);
                    }
                    // Also trace which symbol is at cumulative 238-239
                    if prev_cum <= 238 && cum > 238 {
                        eprintln!("[DS2 pos=13] At cum=238: j={} sym='{}' ({}) prev_cum={} cum={}", 
        ...
        #[cfg(test)]
        if self.debug_count == 0 {
            eprintln!("[DS2 pos={}] esc_freq={} hi_cnt={} scale={} count={} unmasked_idx={}", 
        ...
        if count < hi_cnt {
            ...
            for k in 0..unmasked_idx {
                ...
                if cum > count {
                    ...
                    #[cfg(test)]
                    if self.debug_count == 0 || self.debug_count == 58 {
                        eprintln!("[DS2 pos={}] Selected k={} sym='{}' ({}) at cum={} lo={} freq={}", 
                    ...
                    if self.debug_count == 13 || self.debug_count == 58 {
                        let (code, low, range) = coder.debug_state();
                        eprintln!("[DS2 pos={}] FOUND lo={} hi={} scale={}", self.debug_count, lo_cnt, cum, scale);
                        eprintln!("[DS2 pos={}] Before decode: low={} range={} code={}", self.debug_count, low, range, code);
                    }
                    ...
                    if self.debug_count == 13 || self.debug_count == 58 {
                        let (code, low, range) = coder.debug_state();
                        eprintln!("[DS2 pos={}] After decode: low={} range={} code={}", self.debug_count, low, range, code);
                    }
        ...
        if !is_root {
            self.see2_cont[see2_row][see2_col].summ = 
                self.see2_cont[see2_row][see2_col].summ.wrapping_add(scale as u16);
        }
    ...
    fn create_child(&mut self, parent_ctx: u32, p_stats: usize, first_state_symbol: u8, first_state_freq: u8, first_state_successor: u32) -> u32 {
        ...
        self.write_state(pc as usize + 2, first_state_symbol, first_state_freq, first_state_successor);
    ...
    fn create_successors(&mut self, skip: bool, p1: Option<usize>) -> u32 {
        ...
        if self.debug_count == 12 {
            eprintln!("[CS pos=12] Entry: skip={} p1={:?} up_branch={} fs_symbol='{}'",
    ...
    fn create_successors_finish(&mut self, mut pc: u32, ps: &[usize; MAX_O], pps_idx: usize, up_branch: u32, fs_symbol: u8) -> u32 {
        #[cfg(test)]
        if self.debug_count == 12 {
            eprintln!("[CS_FINISH pos=12] pc={} pps_idx={} up_branch={} fs_symbol='{}'",
                     pc, pps_idx, up_branch, fs_symbol as char);
            for i in 0..pps_idx {
                let sym = self.read_state_symbol(ps[i]);
                eprintln!("[CS_FINISH pos=12] ps[{}]={} sym='{}'", i, ps[i], sym as char);
            }
        }
        ...
        while i > 0 {
            i -= 1;
            pc = self.create_child(pc, ps[i], up_state_symbol, up_state_freq, up_state_successor);
            if pc == 0 {
                return 0;
            }
        }
    ...
    fn update_model(&mut self) {
        ...
        if self.debug_count == 11 || self.debug_count == 12 {
            eprintln!("[UPDATE pos={}] Before: min_context={} max_context={} order_fall={}", 
        ...
        #[cfg(test)]
        if self.debug_count == 12 {
            eprintln!("[UPDATE pos=12] found_state={} fs_sym='{}' fs_freq={} fs_successor={}", 
                     self.found_state, fs_symbol as char, fs_freq, fs_successor);
            let text_ptr = self.sub_alloc.get_text_ptr();
            eprintln!("[UPDATE pos=12] text_ptr={}, fs_successor<=text_ptr: {}", 
                     text_ptr, (fs_successor as usize) <= text_ptr);
        }
        ...
        let s0 = summ_freq.saturating_sub(ns).saturating_sub(fs_freq as u32).saturating_add(1);
        ...
        while pc != self.min_context {
            ...
            if ns1 != 1 {
                ...
                if (ns1 & 1) == 0 {
                    ...
                    let new_stats = self.sub_alloc.expand_units(old_stats as usize, (ns1 >> 1) as usize);
                ...
                let mut sf_inc = 0u16;
                if 2 * ns1 < ns as u16 { sf_inc += 1; }
                let summ = self.read_context_summ_freq(pc as usize);
                if 4 * ns1 as u32 <= ns && summ <= 8 * ns1 { sf_inc += 2; }
            ...
            if cf < 6 * sf {
                ...
                #[cfg(test)]
                if pc == 15728580 {
                    eprintln!("[UPDATE_MODEL pos={}] context {} SummFreq: {} → {} (branch1, sym_freq={})", 
                             self.debug_count, pc, summ, summ + 3, sym_freq);
                }
            } else {
                ...
                #[cfg(test)]
                if pc == 15728580 {
                    eprintln!("[UPDATE_MODEL pos={}] context {} SummFreq: {} → {} (branch2, sym_freq={})", 
                             self.debug_count, pc, summ, summ + sym_freq as u16, sym_freq);
                }
            }
            ...
            #[cfg(test)]
            if ns1 >= 256 {
                eprintln!("[UPDATE] ERROR: Adding state at ns1={} to context {} - exceeds 256!", ns1, pc);
            }
        ...
        #[cfg(test)]
        if self.debug_count == 11 || self.debug_count == 12 {
            eprintln!("[UPDATE pos={}] After: min_context={} max_context={}", 
    ...
    fn write_state_successor(&mut self, offset: usize, successor: u32) {
        ...
        self.sub_alloc.write_byte(offset + 3, (successor >> 8) as u8);
        self.sub_alloc.write_byte(offset + 4, (successor >> 16) as u8);
        self.sub_alloc.write_byte(offset + 5, (successor >> 24) as u8);
    }
