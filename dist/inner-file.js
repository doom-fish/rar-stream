...
export class InnerFile {
    ...
    constructor(name, rarFileChunks) {
        ...
        this.length = rarFileChunks.reduce((length, chunk) => length + chunk.length, 0);
        this.chunkMap = this.calculateChunkMap(rarFileChunks);
        this.name = name;
    }
    readToEnd() {
        return streamToBuffer(this.createReadStream({ start: 0, end: this.length - 1 }));
    }
    getChunksToStream(fileStart, fileEnd) {
        ...
        const first = 0;
        chunksToStream[first] = chunksToStream[first].padStart(Math.abs(startOffset - fileStart));
